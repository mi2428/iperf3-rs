...
// This E2E test exercises the Docker Compose test topology end to end.
//
...
fn compose_interop_and_pushgateway_metrics() {
    ...
    scenario("build shared Docker image", || {
        if !skip_e2e_image_build() {
            project.run_compose(&["build", "client-rs"]);
        }
    });
    scenario("start Compose services", || {
    ...
    scenario("wait for Pushgateway readiness", || {
    ...
    scenario("upstream iperf3 client to iperf3-rs server", || {
    ...
    let first_server_push = scenario("server Pushgateway metrics", || {
    ...
    scenario("iperf3-rs client to upstream iperf3 server", || {
    ...
    scenario(
        "iperf3-rs client to iperf3-rs server with human stdout",
        || {
    ...
    scenario("client TCP Pushgateway metrics", || {
        wait_for_pushgateway_metrics(
            &project,
            CLIENT_SCENARIO,
            &[
    ...
    scenario("server callback persists across client connections", || {
    ...
    scenario("live Pushgateway metrics before client exit", || {
        let live_args = [
            "iperf3-rs",
            "--push.label",
            "scenario=tcp-live",
    ...
    scenario("Prometheus metrics file and Pushgateway fan-out", || {
...
"#,
    ...
    scenario("Pushgateway delete-on-exit removes retained group", || {
        let delete_args = [
            "iperf3-rs",
            "--push.label",
            "scenario=tcp-delete",
    ...
    scenario("window metrics via --push.interval", || {
    ...
    scenario("reverse TCP client metrics", || {
    ...
    scenario("bidirectional TCP client metrics", || {
